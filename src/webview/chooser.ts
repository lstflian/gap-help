...
const CHOOSER_SHIM_JS = (() => {
    try {
        ...
        console.warn(`GAP Help: Failed to load chooser-shim.js: ${e.message}`);
        return '';
    }
})();
...
export function buildChooserShim(backTarget: string, styleValue: string): string {
    ...
    // Strip the header comment, then replace both placeholders in one pass.
    // The page must start with the IIFE, not a comment.
    // A value with a placeholder literal cannot break the other.
