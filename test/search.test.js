 ...
 *
 * Usage: node test/search.test.js   (run npm run compile first)
 */
...
const { searchHelp } = require(path.join(OUT, 'searchEngine.js'));
const { loadIndex } = require(path.join(OUT, 'indexData.js'));
...
const dataDir = path.join(__dirname, '..', 'indexdata', '4.16.0');
